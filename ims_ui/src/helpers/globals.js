...
export const getErrMessage = (error) => {
  const defErrMsg = 'Something went wrong! No proper message!'
  if (!error) return defErrMsg
  const displayErrMsg = error.response?.data?.displayMessage
...
export const findSolution = (a, b, n) => {
  let counter = 0

  const calculateArrays = (a, b, n) => {
    const newArray = []
    for (let i = 0; i < n; i++) {
  ...
  const isEligible = (a, b) => {
    const res = a.some((i, idx) => i < b[idx])
    return !res
  }

  const recursiveFn = (a, b, n) => {
    if (isEligible(a, b)) {
      counter++
      let newA = calculateArrays(a, b, n)
      recursiveFn(newA, b, n)
    }
    return counter
  }

  const result = recursiveFn(a, b, n)

  return result
}

export const findFibonacci = (n) => {
  const inits = [0, 1]

  for (let i = 2; i < n; i++) {
    inits[i] = inits[i - 2] + inits[i - 1]
  }
